...
int find(int node, int start, int end, int left, int right, bool maximum) {
    ...
    int mid = start + (end - start) / 2;
    if (maximum) return max(find(node * 2, start, mid, left, right, maximum), find(node * 2 + 1, mid + 1, end, left, right, maximum));
    return min(find(node * 2, start, mid, left, right, maximum), find(node * 2 + 1, mid + 1, end, left, right, maximum));
}
...
int main() {
    ...
    cin.tie(NULL);
    cout.tie(NULL);
    int num, height, q, big, small, a, b;
    cin >> num >> q;
    ...
    for (int i = 0; i < q; i++) {
        ...
        big = a > b ? a : b;
        small = a > b ? b : a;
        cout << find(1, 0, num - 1, small - 1, big - 1, false) << " " << find(1, 0, num - 1, small - 1, big - 1, true) << '\n';
    }
    return 0;
}
